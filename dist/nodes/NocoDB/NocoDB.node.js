...
function buildWhere(groups) {
    if (!groups || !groups.length)
        return "";
    // groups: [{logic:'and'|'or', conditions:[{field,operator,value}]}]
    const parts = [];
...
function normalizeFieldsCollection(entries, tableId) {
    const seen = new Set();
    const payload = {};
    // `entries` shapes:
    // - array of {fieldValues:{field,value}} (multiple rows)
    // - object {fieldValues:[{field,value},...]} (single group)
    // - object {fieldValues:{field,value}} (single group, single condition)
    // - object {field,value} (defensive)
    let list = [];
    if (!entries) {
        list = [];
    }
    ...
    for (const record of records) {
        for (const link of linkDefs) {
            ...
            try {
                ...
                // overwrite any scalar/count value that API may have returned
                record.fields[titleKey] = payload;
                // drop the technical key if it's different from title to avoid duplicates in final output
...
class NocoDB {
    constructor() {
        this.description = {
            displayName: "NocoDB (Custom)",
            name: "nocodbCustom",
            icon: "file:nocodb-node-logo.svg",
            group: ["input"],
            version: 1,
            subtitle: '={{$parameter["operation"]}}',
            description: "Work with NocoDB rows",
            defaults: {
                name: "NocoDB",
            },
            inputs: ["main"],
        ...
        this.methods = {
            ...
            loadOptions: {
                ...
                async getLinkFields() {
                    ...
                    if (!tableId)
                        return [];
                    // fetch columns so that value is id (needed for links API)
                    let columns = [];
                    try {
                        const baseParam = this.getNodeParameter("baseId", 0);
                        const baseId = (0, utils_1.resolveId)(baseParam);
                        const response = await (0, utils_1.apiRequest)(this, "GET", `/api/v3/meta/bases/${baseId}/tables/${tableId}`);
                        columns = response.fields || response.columns || [];
                    }
                    catch (error) {
                        const response = await (0, utils_1.apiRequest)(this, "GET", `/api/v1/db/meta/tables/${tableId}`);
                        columns = response.columns || [];
                    }
                    return columns
                        .filter((c) => c.uidt === "LinkToAnotherRecord")
                        .map((c) => ({
                        name: c.title || c.column_name || c.id,
                        value: c.id,
                        description: c.uidt,
                    }));
                },
            ...
            try {
                if (normalizedResource === "row") {
                    ...
                    if (operation === "create") {
                        ...
                        console.log("NocoDB create payload", { baseId: baseIdResolved, tableId: tableIdResolved, payloadArray });
                    ...
                    if (operation === "update") {
                        ...
                        console.log("NocoDB update payload", { baseId: baseIdResolved, tableId: tableIdResolved, payload });
                    ...
                    if (operation === "get") {
                        ...
                        console.log("NocoDB get request", { baseId: baseIdResolved, tableId: tableIdResolved, recordId, qs });
                    ...
                    if (operation === "getAll") {
                        ...
                        console.log("NocoDB getAll request", {
                            baseId: baseIdResolved,
                    ...
                    if (operation === "count") {
                        ...
                        console.log("NocoDB count request", {
                            baseId: baseIdResolved,
                            tableId: tableIdResolved,
                            filterGroups,
                            qs,
